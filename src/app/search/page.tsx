...
const Page = ({ params }: Props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/search/?query=${search}`);
        setCategories(response.data.categories);
        setBusinesses(response.data.businesses);
        setLoading(false);

        // If there are no businesses but there are categories, set the active tab to categories
        if (response.data.businesses.length === 0 && response.data.categories.length > 0) {
        ...
        <div className="flex flex-row max-lg:flex-col w-full gap-6 items-start mt-6">
          <div className="w-1/4 flex flex-col gap-4 items-end max-lg:w-full max-md:items-end mt-6 md:flex-row">
            <DropdownMenu>
              <DropdownMenuTrigger>
                <div
                  className="border-2 border-secondary flex flex-row gap-4 items-center text-xs justify-center w-fit px-4 h-10"
                >
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onSelect={() => { handleFilterChange('Recommended'); }}>Recommended</DropdownMenuItem>
                <DropdownMenuItem onSelect={() => { handleFilterChange('Most Reviewed'); }}>Most Reviewed</DropdownMenuItem>
                <DropdownMenuItem onSelect={() => { handleFilterChange('Most Rated'); }}>Most Rated</DropdownMenuItem>
                <DropdownMenuItem onSelect={() => { handleFilterChange('Alphabetical Order'); }}>Alphabetical Order</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
            ...
            <>
              {businesses.length === 0 && categories.length === 0 ? (
                <div className='flex flex-col h-full w-full gap-4 justify-center items-center'>
                  <Typography variant="h3">No results found for ' {search} '</Typography>
                  Try searching again <div className=''><SearchComponent/></div>
                </div>
              ) : (
                <>
                  {activeTab === 'businesses' && businesses.length > 0 && (
                    <div>
                      {filteredBusinesses(businesses).map((item: { name?: any; tags?: any; hours?: any; description?: any; average_rating?: any; review_count?: any; price_range?: any; slug?: string; }) => (
                        <div
                          key={item.name}
                          onClick={() => { handleBusinessClick(item.slug!); }}
        ...
        >
          <p className='whitespace-nowrap text-sm'>{category.name}</p>
                      ...
                      {selectedCategory && selectedCategory.businesses.length > 0 && (
                        <div className="mt-6">
                          {filteredBusinesses(selectedCategory.businesses).map((item: { name?: any; tags?: any; hours?: any; description?: any; average_rating?: any; price_range?: any; review_count?: any; slug?: string; }) => (
                            <div
                              key={item.name}
                              onClick={() => { handleBusinessClick(item.slug!); }}
                              className="cursor-pointer"
